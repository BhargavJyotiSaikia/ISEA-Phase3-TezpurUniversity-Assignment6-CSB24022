...
def receive():
    ...
    while True:
        try:
            data = client.recv(MAX_MESSAGE_SIZE).decode()
            ...
            auth_status_label.config(text="Reconnecting...", fg="orange")
...
def connect_server():
    ...
    try:
        ...
        server_msg = client.recv(MAX_MESSAGE_SIZE).decode()
        ...
        response = client.recv(MAX_MESSAGE_SIZE).decode()
        ...
        if response.startswith("AUTH_FAILED:"):
            ...
            if login_attempts >= max_attempts:
                blocked_until = time.time() + LOGIN_BLOCK_DURATION
